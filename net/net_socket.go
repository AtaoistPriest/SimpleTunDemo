...
/* socket将在timeVal时间间隔内接收数据，超出时间后返回失败。 */
func RecvFromDontWait(fd int, recvBuff []byte, timeVal time.Duration) (int, *unix.SockaddrInet4, error){
	crycle := time.Millisecond * 10
	cnt := timeVal / crycle
	for cnt > 0{
		dataLen, from, err := unix.Recvfrom(fd, recvBuff, unix.MSG_DONTWAIT)
		if dataLen > 0{
			fromAddr, _ := from.(*unix.SockaddrInet4)
			return dataLen, fromAddr, err
		}
